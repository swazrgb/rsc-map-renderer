...
public final class World {
	...
	public void setSectorProvider(SectorProvider provider) {
		this.sectorProvider = provider;
	}

	/**
	 * Remove all building-roof model chunks from the scene, across every floor.
	 * The stock client hides roofs when the player is underneath; for an
	 * overhead map we drop them entirely so building interiors/footprints are
	 * visible rather than rooftops. Mirrors the {@code scene.removeModel(
	...
	public void removeRoofs(int floor) {
		...
		// own walls (modelWallGrid[floor]) so building footprints stay visible.
		for (int offset = 0; offset <= 2; offset++) {
			int f = floor + offset;
			if (f >= modelRoofGrid.length) {
				break;
			}
			removeGrid(modelRoofGrid[f]);
			if (offset >= 1) {
				removeGrid(modelWallGrid[f]);
			}
		}
	}

	/**
	 * Remove all wall geometry from the scene. In a straight-down view vertical
	 * walls are edge-on (they render as thin lines / shading — "wall shadows");
	 * the map instead draws walls as clean lines via {@link #collectWallSegments}.
	 */
	public void removeAllWalls() {
		for (RSModel[] grid : modelWallGrid) {
			removeGrid(grid);
		}
	}

	/**
	 * Remove the terrain (ground) model chunks from the scene. For isolated
	 * object-sprite renders only the placed scenery model should rasterize, so
	 * the frame background stays at the sentinel colour (→ alpha).
	 */
	public void removeLandscape() {
		removeGrid(modelLandscapeGrid);
	}

	// --- Chunk-model views for RegionExporter (browser-viewer geometry bake). ---
	RSModel[] landscapeChunks() {
		return modelLandscapeGrid;
	}

	RSModel[] wallChunks(int floor) {
		return modelWallGrid[floor];
	}

	RSModel[] roofChunks(int floor) {
		return modelRoofGrid[floor];
	}

	/**
	 * Place one scenery object's model into the scene EXACTLY the way the live
	 * client spawns scenery (PacketHandler opcode-48 object spawn,
	 * {@code Client_Base-teavm/src/main/java/orsc/PacketHandler.java:1487-1518}):
	 * {@code clone()} → {@code addRotation(0, dir*32, 0)} → {@code translate2}
	 * to the footprint centre at terrain elevation →
	 * {@code setDiffuseLightAndColor(-50, -10, -50, 48, 48, true, 117)}, plus
	 * the id-74 fire vertical offset. This is the recipe whose top-down look was
	 * validated in the live client at pitch 768; no double-siding, no light
	 * overrides.
	 *
	 * @return the placed scene model (remove it with {@code scene.removeModel}
	 *     when done), or null when the def/model is missing or empty
	 */
	public RSModel placeObjectClient(int id, int localX, int localZ, int direction) {
		var def = EntityHandler.getObjectDef(id);
		return def == null ? null
			: placeObjectClient(id, localX, localZ, direction, def.modelID);
	}

	/** As {@link #placeObjectClient(int, int, int, int)} but with an explicit model — the
	 *  client's scenery animation swaps an object's model per frame (firea1..3, torcha1..4)
	 *  while keeping its placement recipe (mudclient_update_object_animation). */
	public RSModel placeObjectClient(int id, int localX, int localZ, int direction, int modelId) {
		if (modelCache == null) {
			return null;
		}
		var def = EntityHandler.getObjectDef(id);
		if (def == null || modelId < 0 || modelId >= modelCache.length) {
			return null;
		}
		RSModel base = modelCache[modelId];
		if (base == null || base.vertHead <= 1) {
			return null;
		}
		int xSize;
		int zSize;
		...
		int xWorld = (localX * 2 + xSize) * 128 / 2;
		int zWorld = (localZ * 2 + zSize) * 128 / 2;
		RSModel m = base.clone();
		this.scene.addModel(m);
		m.addRotation(0, direction * 32, 0);
		m.translate2(xWorld, -this.getElevation(xWorld, zWorld), zWorld);
	...
	public void removeModel(RSModel m) {
		this.scene.removeModel(m);
	}

	/**
	 * Collect the region's wall segments as tile-corner endpoints
	 * {@code [x1, z1, x2, z2]} (corner coords, ×128 = world units), mirroring the
	 * wall geometry {@code generateLandscapeModel} would build. Used to draw a
	 * floor-plan-style wall overlay on the overhead map.
	 */
	public java.util.List<int[]> collectWallSegments() {
		java.util.List<int[]> segs = new java.util.ArrayList<>();
		for (int x = 0; x < 95; x++) {
			for (int z = 0; z < 95; z++) {
				if (isVisibleWall(getVerticalWall(x, z))) {
					segs.add(new int[]{x, z, x + 1, z}); // runs east-west
				}
				if (isVisibleWall(getHorizontalWall(x, z))) {
					segs.add(new int[]{x, z, x, z + 1}); // runs north-south
				}
				int d = getWallDiagonal(x, z);
				if (d > 0 && d < 12000 && isVisibleDoor(d - 1)) {
					segs.add(new int[]{x, z, x + 1, z + 1});
				} else if (d > 12000 && d < 24000 && isVisibleDoor(d - 12001)) {
					segs.add(new int[]{x + 1, z, x, z + 1});
				}
			}
		}
		return segs;
	...
	private boolean isVisibleWall(int wall) {
		return wall > 0 && isVisibleDoor(wall - 1);
	}

	private boolean isVisibleDoor(int doorId) {
		var def = EntityHandler.getDoorDef(doorId);
		return def != null && def.getUnknown() == 0;
	}

	/** Diagnostic: remove arbitrary roof/wall grids by floor (true = remove). */
	...
	private void removeGrid(RSModel[] grid) {
		for (RSModel chunk : grid) {
			if (chunk != null) {
				this.scene.removeModel(chunk);
			}
		}
	}
	...
	/** Diagnostic: dump a scenery model's geometry to classify it (billboard vs 3D). */
	public void debugModelExtents(int objectId) {
		var def = EntityHandler.getObjectDef(objectId);
		if (def == null) { System.out.println("obj " + objectId + " null def"); return; }
		RSModel m = modelCache[def.modelID];
		if (m == null) { System.out.println("obj " + objectId + " null model " + def.modelID); return; }
		int minX = Integer.MAX_VALUE, maxX = Integer.MIN_VALUE;
		int minZ = Integer.MAX_VALUE, maxZ = Integer.MIN_VALUE;
		for (int i = 0; i < m.vertHead; i++) {
			minX = Math.min(minX, m.vertX[i]); maxX = Math.max(maxX, m.vertX[i]);
			minZ = Math.min(minZ, m.vertZ[i]); maxZ = Math.max(maxZ, m.vertZ[i]);
		}
		System.out.println("obj " + objectId + " modelID=" + def.modelID
			+ " verts=" + m.vertHead + " faces=" + m.faceHead
	...
	public World(Scene var1, GraphicsController var2) {
		try {
			...
			try {
				...
				// Authentic_Landscape.orsc is fine. The ZIP path is only used when no
				// sector provider has been set.
				tileArchive = null;
			}
				...
				if (showWallOnMinimap) {
					...
					for (int x = 0; x < 96; ++x)
						for (int z = 0; z < 96; ++z) {
							...
							// Stock client rolls Math.random() here — per-load noise. We
							// seed the same ±5 grain from the WORLD tile position so two
							// loads covering the same tiles produce identical terrain
							// lighting (required for seam-free mesh export; also makes
							// renders deterministic). var1/var5 = load-centre world tile.
							int wx = ((24 + var1) / 48 - 1) * 48 + x;
							int wz = ((24 + var5) / 48 - 1) * 48 + z;
							int h = wx * 73856093 ^ wz * 19349663 ^ plane * 83492791;
							h ^= h >>> 13;
							int val = (h >>> 4) % 10 - 5;
	...
	/**
	 * Resolve one landscape section. Prefers the injected {@link SectorProvider}
	 * (JAG terrain); falls back to the {@code Authentic_Landscape.orsc} ZIP.
	 */
	private Sector fetchSection(int height, int sectionX, int sectionY) {
		if (sectorProvider != null) {
			Sector s = sectorProvider.getSection(height, sectionX, sectionY);
			return s != null ? s : blankSection(height);
		}
		try {
			String filename = "h" + height + "x" + sectionX + "y" + sectionY;
			ZipEntry e = tileArchive.getEntry(filename);
			if (e == null) {
				return blankSection(height);
			}
	...
	/** A blank sector, with the default ground overlay the ZIP path used for absent sections. */
	private static Sector blankSection(int height) {
		Sector s = new Sector();
