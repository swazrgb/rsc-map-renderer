...
public final class RSModel {
	...
	/**
	 * Make every face render from both sides by mirroring a present texture/
	 * colour onto a {@link Scene#TRANSPARENT} opposite side. RSC scenery faces
	 * are single-sided (authored to be seen from outside / at an angle), so the
	 * up-facing tops are culled when viewed straight down — a tree collapses to
	 * its visible underside. Double-siding makes scenery render from any angle,
	 * including directly overhead, without per-model hacks.
	 */
	public final void makeDoubleSided() {
		for (int i = 0; i < this.faceHead; i++) {
			if (this.faceTextureBack[i] == Scene.TRANSPARENT
				&& this.faceTextureFront[i] != Scene.TRANSPARENT) {
				this.faceTextureBack[i] = this.faceTextureFront[i];
			} else if (this.faceTextureFront[i] == Scene.TRANSPARENT
				&& this.faceTextureBack[i] != Scene.TRANSPARENT) {
				this.faceTextureFront[i] = this.faceTextureBack[i];
			}
		}
	}

	/**
	 * Diagnostic: looking straight down the model's central axis, what is the
	 * highest face that actually covers the centre column? If the canopy is an
	 * open umbrella shell (authored for an oblique view), nothing covers the
	 * apex and a straight-down ray falls through to the trunk — explaining the
	 * "ring with the trunk showing through" look.
	 */
	public final void debugTopDownCoverage(int objectId) {
		long sx = 0, sz = 0;
		...
		int cx = (int) (sx / Math.max(1, this.vertHead));
		int cz = (int) (sz / Math.max(1, this.vertHead));
		// RSC: up is -Y. Find highest (most negative Y) face whose XZ polygon
		// contains the centre column (cx,cz).
		int coverY = Integer.MAX_VALUE; // most-negative covering face avgY
		int top20faces = 0, top20covering = 0;
		int yTopBand = minY + (maxY - minY) / 5; // top fifth (most negative)
		for (int f = 0; f < this.faceHead; f++) {
			int[] vs = this.faceIndices[f];
			long ay = 0;
			for (int v : vs) ay += this.vertY[v];
			int avgY = (int) (ay / vs.length);
			boolean inTop = avgY <= yTopBand;
			if (inTop) top20faces++;
			if (pointInProjectedFace(cx, cz, vs)) {
				if (avgY < coverY) coverY = avgY;
				if (inTop) top20covering++;
			}
		}
		System.out.println("    coverage obj " + objectId
			+ " Yrange[" + minY + ".." + maxY + "] (up=-Y)"
			+ " centre=(" + cx + "," + cz + ")"
			+ " highestFaceCoveringCentre.avgY=" + (coverY == Integer.MAX_VALUE ? "NONE" : coverY)
	...
	private boolean pointInProjectedFace(int px, int pz, int[] vs) {
		...
		for (int i = 0, j = vs.length - 1; i < vs.length; j = i++) {
			int xi = this.vertX[vs[i]], zi = this.vertZ[vs[i]];
			int xj = this.vertX[vs[j]], zj = this.vertZ[vs[j]];
			if (((zi > pz) != (zj > pz))
		...
		try {
			...
			if (this.minZ <= MiscFunctions.frustumMinX && this.maxZ >= MiscFunctions.frustumMaxX
				...
				for (int var17 = 0; this.vertHead > var17; ++var17) {
					...
					if (orthoScale != 0) {
						// Orthographic projection: screen pos is independent of
						// depth (no /zO), so there is zero parallax — a true
						// top-down view. orthoScale is 8.8 fixed-point world-to-
						// screen scale (px per world unit << 8). Depth (vertZRot)
						// is still kept below for sorting, fog and frustum cull.
						this.vertexParam6[var17] = x0 * orthoScale >> 8;
						this.vertexParam2[var17] = yO * orthoScale >> 8;
					} else if (zO < zTop) {
	...
	/**
	 * Per-axis scale in 256ths, applied in the transform pipeline between the
	 * dir-rotation and the translate (so a Y squash keeps the base on the
	 * ground and is unaffected by facing). Used by the top-down sprite renderer
	 * to flatten model height instead of raising the camera — high focal
	 * shifts destabilize the texturer, squashed geometry does not.
	 */
	public final void setScale256(int sx, int sy, int sz) {
		this.scaleX = sx;
		this.scaleY = sy;
		this.scaleZ = sz;
		this.computeAppliedTransform();
	...
	// --- Read-only views for MeshExporter (browser-viewer geometry bake). ---
	int rot256YForExport() {
		return rot256Y;
	}

	int[] scale256ForExport() {
		return new int[]{scaleX, scaleY, scaleZ};
	}

	int[] translateForExport() {
		return new int[]{translateX, translateY, translateZ};
	}

	int diffuseParam1ForExport() {
		return diffuseParam1;
	}
	...
	/**
	 * Recompute diffuse intensities from TRANSFORMED coordinates without
	 * mutating the model — the engine relights on every transform apply
	 * ({@link #resetTransformCache} → {@link #computeNormals} →
	 * {@link #computeDiffuse}), so for placed scenery the stored
	 * faceDiffuseLight/vertDiffuseLight are stale recipe-time values computed
	 * from unrotated normals. Exact integer replica of those two methods.
	 *
	 * @return {faceIntensity[faceHead], vertIntensity[vertHead]}; null when the
	 *     model doesn't relight (dontComputeDiffuse).
	 */
	int[][] relitIntensitiesForExport(int[] tx, int[] ty, int[] tz) {
		if (this.dontComputeDiffuse) {
			return null;
		}
		int[] fNormX = new int[this.faceHead];
		int[] fNormY = new int[this.faceHead];
		int[] fNormZ = new int[this.faceHead];
		for (int face = 0; face < this.faceHead; ++face) {
			int[] vs = this.faceIndices[face];
			int xp = tx[vs[0]];
			int yp = ty[vs[0]];
			int zp = tz[vs[0]];
			int x21 = tx[vs[1]] - xp;
			...
			fNormX[face] = xN * 65536 / mag;
			fNormY[face] = yN * 65536 / mag;
			fNormZ[face] = zN * 65535 / mag; // engine quirk: 65535 for Z
		}

		int diffuseDivide = this.diffuseParam2 * this.diffuseMag >> 8;
		int[] faceIntensity = new int[this.faceHead];
		for (int f = 0; f < this.faceHead; ++f) {
			if (this.faceDiffuseLight[f] != this.m_Vb) {
				faceIntensity[f] = (fNormY[f] * this.diffuseDirY
					+ fNormX[f] * this.diffuseDirX + this.diffuseDirZ * fNormZ[f])
					/ diffuseDivide;
			} else {
				faceIntensity[f] = this.m_Vb;
			}
		}

		int[] sumX = new int[this.vertHead];
		int[] sumY = new int[this.vertHead];
		int[] sumZ = new int[this.vertHead];
		int[] count = new int[this.vertHead];
		for (int f = 0; f < this.faceHead; ++f) {
			if (this.faceDiffuseLight[f] == this.m_Vb) {
				for (int fi = 0; fi < this.faceIndexCount[f]; ++fi) {
					int v = this.faceIndices[f][fi];
					sumX[v] += fNormX[f];
					sumY[v] += fNormY[f];
					sumZ[v] += fNormZ[f];
					++count[v];
		...
		int[] vertIntensity = new int[this.vertHead];
		for (int v = 0; v < this.vertHead; ++v) {
			vertIntensity[v] = this.vertDiffuseLight[v];
			if (count[v] > 0) {
				vertIntensity[v] = (sumZ[v] * this.diffuseDirZ + sumX[v] * this.diffuseDirX
					+ sumY[v] * this.diffuseDirY) / (diffuseDivide * count[v]);
			}
		}
		return new int[][]{faceIntensity, vertIntensity};
	}
