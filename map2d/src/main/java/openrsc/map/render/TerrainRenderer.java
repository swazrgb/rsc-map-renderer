...
public final class TerrainRenderer {
  ...
  public static BufferedImage render(int floor, CollisionMap cm, TileDefs tileDefs) {
    return render(floor, cm, tileDefs, false);
  }

  /**
   * Render the terrain layer, optionally washed out. When {@code dim} is true every tile colour is
   * halved per channel ({@code (rgb >> 1) & 0x7F7F7F}) — the exact shading the stock client applies
   * in {@code drawMinimapTile}, so base+walls reproduces the in-game minimap's washed-out look. When
   * false the palette renders at full saturation (the brighter web-map look). Dimming affects only
   * this base terrain layer; the walls/blocked overlays are solid colours and are never touched.
   */
  public static BufferedImage render(int floor, CollisionMap cm, TileDefs tileDefs, boolean dim) {
    return render(floor, cm, tileDefs, dim, 1);
  }

  /**
   * As {@link #render(int, CollisionMap, TileDefs, boolean)} but at {@code mult}× the native
   * {@link #SCALE} px/tile (the {@code -Dmap.scale} multiplier). {@code mult == 1} is the exact
   * output baked by default; higher values give a bigger, crisper raster — the terrain is flat per
   * tile, so this enlarges the blocks, it does not add sub-tile detail.
   */
  public static BufferedImage render(int floor, CollisionMap cm, TileDefs tileDefs, boolean dim,
      int mult) {
    long t0 = System.nanoTime();
    int s = SCALE * Math.max(1, mult);
    ...
    LOG.info("Terrain floor {}: {}×{} px ({}× scale), {} overlay tiles ({} ms)",
        floor, w * s, h * s, s, overlayCount, ms);
    return img;
  }
  ...
  public static byte[] renderPng(int floor, CollisionMap cm, TileDefs tileDefs) {
    return renderPng(floor, cm, tileDefs, false, 1);
  }

  /** PNG-encode {@link #render(int, CollisionMap, TileDefs, boolean)}'s output. */
  public static byte[] renderPng(int floor, CollisionMap cm, TileDefs tileDefs, boolean dim) {
    return renderPng(floor, cm, tileDefs, dim, 1);
  }

  /** PNG-encode {@link #render(int, CollisionMap, TileDefs, boolean, int)}'s output. */
  public static byte[] renderPng(int floor, CollisionMap cm, TileDefs tileDefs, boolean dim,
      int mult) {
    BufferedImage img = render(floor, cm, tileDefs, dim, mult);
