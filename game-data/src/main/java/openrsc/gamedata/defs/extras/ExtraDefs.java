 ...
 * graph via {@link openrsc.gamedata.runtime.GameEnvironment#ladderTelepointsXml()}), the
 * {@code retro/} subtree, and {@code CombatOdyssey.json} (not XML, not loaded by EntityHandler).
 *
 * <p>One table lives a level up from {@code defs/extras}: {@link SpellDefs} loads
 * {@code defs/SpellDef.xml} (same XStream shape, same EntityHandler owner) — bundled here so every
 * {@code ExtraDefs.load(conf)} caller gets spell names without a separate loader to wire.
 */
public record ExtraDefs(
  ...
  public static ExtraDefs load(ServerConf conf) {
    ...
    return new ExtraDefs(
        ...
        SpellDefs.load(conf.defs().resolve("SpellDef.xml")));
  }
