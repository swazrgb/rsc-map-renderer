...
public final class WallOverlay {
  ...
  /**
   * Overlay flag bytes for in-grid tiles, stored densely per 8×8 chunk: {@code chunkTiles[chunk]} is
   * a lazily-allocated {@code byte[64]} (local index {@code ((y&7)<<3)|(x&7)}), null when the chunk
   * has never held a flag. This replaces a {@code ConcurrentHashMap<Long,Byte>} whose
   * {@code get(key(x,y))} boxed a {@code Long} on every read that passed the {@link #chunkBits}
   * guard — that autobox was 55% of ALL pathfinding allocation (JFR, 2026-07): the pathfinder reads
   * {@link #flags} for every tile it expands, and any populated chunk (a nearby door/wall) fell
   * through to the boxed lookup. The dense array is boxing-free and cache-friendly.
   *
   * <p>Plain {@code byte[][]}, no atomics: same single-writer (account loop) / benign-stale-read
   * contract as {@link #chunkBits} — a cross-thread reader that races the writer sees a null slot or
   * a not-yet-published byte as 0 (static flags only), the same benign staleness the pre-guard CHM
   * read already tolerated. An {@code AtomicReferenceArray} was rejected for the same reason the
   * guard avoids {@code AtomicLongArray}: it failed to inline and dominated samples.
   */
  private final byte[][] chunkTiles = new byte[CHUNKS_X * CHUNKS_Y][];

  /**
   * Out-of-grid overlay flags only (e.g. {@code or(x-1, …)} at x=0 writes the WALL_WEST of the
   * off-map neighbour). Rare and never on the hot expansion path, so the boxed-key CHM is fine here;
   * the dense {@link #chunkTiles} serves every in-grid read boxing-free.
   */
  private final ConcurrentHashMap<Long, Byte> oobTiles = new ConcurrentHashMap<>();

  private static int localIndex(int x, int y) {
    return ((y & 7) << 3) | (x & 7);
  }
  ...
  public int flags(int x, int y) {
    if (inGrid(x, y)) {
      ...
      byte[] ct = chunkTiles[chunk];
      return ct == null ? 0 : ct[localIndex(x, y)] & 0xFF;
    }
    Byte b = oobTiles.get(key(x, y));
    return b == null ? 0 : b & 0xFF;
  }

  public int size() {
    int n = oobTiles.size();
    for (byte[] ct : chunkTiles) {
      if (ct != null) {
        for (byte b : ct) {
          if (b != 0) {
            n++;
  ...
  /**
   * Order-independent content hash of every non-zero overlay tile inside the inclusive bbox, or 0
   * iff the window holds no overlay flag at all. Never returns 0 for a non-empty window, so callers
   * can use 0 as "no overlay here" and hoist the overlay reference entirely — with per-TILE
   * precision, unlike {@link #intersects}, whose chunk-granular guard bits are sticky
   * ({@link #andNot} never clears them), so a door that once existed anywhere in a 8×8 chunk makes
   * it report walls forever.
   *
   * <p>This exists for cache KEYS (the pathfinder's cluster-leg cache): a leg is a pure function of
   * the wall content inside its window, so keying on this signature lets a whole swarm of bots —
   * each with its own overlay instance and its own {@link #generation} churn — share one cached leg
   * whenever their observed walls match, and keeps a cached leg valid across mutations that happen
   * OUTSIDE the window. Same clamped-to-grid semantics as {@code intersects}: out-of-grid
   * ({@link #oobTiles}) entries are ignored, matching the existing hoist behaviour.
   */
  public long signature(int x0, int y0, int x1, int y1) {
    int gx0 = Math.max(0, x0), gx1 = Math.min(CollisionMap.WIDTH - 1, x1);
    int gy0 = Math.max(0, y0), gy1 = Math.min(CollisionMap.HEIGHT - 1, y1);
    long h = 0;
    boolean found = false;
    for (int cy = gy0 >> 3; cy <= gy1 >> 3; cy++) {
      for (int cx = gx0 >> 3; cx <= gx1 >> 3; cx++) {
        int chunk = cy * CHUNKS_X + cx;
        if ((chunkBits[chunk >> 6] & (1L << chunk)) == 0) {
          continue;
        }
        ...
        int bx = cx << 3, by = cy << 3;
        int ly0 = Math.max(gy0 - by, 0), ly1 = Math.min(gy1 - by, 7);
        int lx0 = Math.max(gx0 - bx, 0), lx1 = Math.min(gx1 - bx, 7);
        for (int ly = ly0; ly <= ly1; ly++) {
          for (int lx = lx0; lx <= lx1; lx++) {
            int f = ct[(ly << 3) | lx] & 0xFF;
            if (f != 0) {
              found = true;
              h ^= mix64(((long) (bx + lx) << 24) | ((long) (by + ly) << 8) | f);
            }
          }
        }
      }
    }
    return found && h == 0 ? 1 : h;
  }

  /** SplitMix64 finalizer — full-avalanche mix for {@link #signature}'s per-tile terms. */
  ...
  public void clearChunk(int chunkX, int chunkZ) {
    ...
    if (chunkX >= 0 && chunkX < CHUNKS_X && chunkZ >= 0 && chunkZ < CHUNKS_Y) {
      ...
      chunkTiles[chunk] = null;
      chunkBits[chunk >> 6] &= ~(1L << chunk);
    }
    // Out-of-grid entries were never matched by the old (x>>3)==chunkX test (chunkX ≥ 0), so
    // oobTiles is intentionally left untouched — behaviour-preserving.
  }
    ...
    for (int y = y0; y <= y1; y++) {
      for (int x = x0; x <= x1; x++) {
        ...
        sb.append("  (").append(x).append(',').append(y).append(") = 0x")
            .append(String.format("%02x", f)).append('\n');
        if (++n > 200) {
          sb.append("  ...truncated\n");
          break outer;
        }
  ...
  private void or(int x, int y, int mask) {
    if (inGrid(x, y)) {
      // Set the guard bit BEFORE the array write so a racing reader can
      ...
      int li = localIndex(x, y);
      ct[li] = (byte) ((ct[li] & 0xFF) | (mask & 0xFF));
      return;
    }
    oobTiles.merge(key(x, y), (byte) mask,
        (oldVal, addVal) -> (byte) ((oldVal & 0xFF) | (addVal & 0xFF)));
  }

  private void andNot(int x, int y, int mask) {
    if (inGrid(x, y)) {
      // Leaves the chunk bit set — a stale positive costs one array read, never correctness.
      byte[] ct = chunkTiles[chunkIndex(x, y)];
      if (ct != null) {
        int li = localIndex(x, y);
        ct[li] = (byte) ((ct[li] & 0xFF) & ~mask);
      }
      return;
    }
    oobTiles.computeIfPresent(key(x, y), (k, v) -> {
      int nv = (v & 0xFF) & ~mask;
