...
public interface ServerData {
   ...
   * (see {@code openrsc.gamedata.NpcLocs}). The server walks a roaming NPC to a random tile
  ...
  /**
   * Spell definition for a wire spell id (the positional index into the server's
   * {@code defs/SpellDef.xml} — the non-retro table Uranium loads), or {@code null} when out of
   * range (server {@code EntityHandler.getSpellDef}).
   */
  Spell spell(int spellId);

  record Spell(int id, String name, int requiredLevel) {
